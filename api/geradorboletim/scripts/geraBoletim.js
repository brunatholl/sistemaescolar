$(document).ready(function () {
    ...
    .done(function (data) {
        var projectList = jQuery.parseJSON(data);

        if (projectList.length > 0) {
            projectList.forEach(function (element, index) {

                //remove dir/ and .csv of name of file
                var nameFileSplit = element.split("//")[1].split(".")[0];
                nameFileSplit = nameFileSplit.replace(/_/g, " ");

                var bt = "<li class='list-group-item'><span class='name-list-item'><b>" + nameFileSplit + "</b></span>";
                ...
                $(".bt-gerate-notes").click(function () {
                    gerarBoletim($(this).attr("dirfile"));
                });
            });
...
function gerarBoletim(data) {

    var disciplines = ["Português", "Redação", "Inglês", "Espanhol",
                ...
                if (rawFile.status === 200 || rawFile.status == 0) {
                    ...
                    for (var i = 7; i < arrText.length; i++) {
                        ...
                        if (arrText[i] != "") {
                            ...
                            var arrTextSplit = arrText[i].split(";");
                            //is empyt?
                            ...
                            //name of student
                            var name_students = arrTextSplit[1];
                            ...
                            //remove dir/ and .csv of name of file
                            var nameFileSplit = namefile.split("//")[1].split(".")[0];

                            //slpiting year, class and shift
                            var ano_turma_turno = nameFileSplit.split("_");
                            var ano = ano_turma_turno[0];
                            var turma = ano_turma_turno[1];
                            var turno = ano_turma_turno[2];
                        }
                    }
...
/*@function saveData
 @abstract this function get datas about course, students e notes and send for input in html
 */
